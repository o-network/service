...
export class IndexedFormula extends Formula {
  ...
  query(query: Query, callback: (value: UpdateClauses) => void, fetcher: any, onDone: (error?: Error) => void): void;

  applyPatch(patch: UpdateClauses, url: NamedNode, onDone: (error?: Error) => void): void;
}
...
export type UpdateClauses = { [key: string]: unknown };

export function sparqlUpdateParser(str: string, kb: Formula, base: string): UpdateClauses;

export function SPARQLToQuery(sparql: string, testMode: boolean, kb: Formula): any;

export type SerializeOptions = {
  flags?: string
};

export function serialize(resource: NamedNode, kb: Formula, url: string, contentType: string, callback?: (error?: Error, value?: string) => void, options?: SerializeOptions): void | string;
